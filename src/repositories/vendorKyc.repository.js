...
// Flatten the `serviceCategories` join rows into a plain `services` array of
// { id, name, slug } — callers shouldn't need to know about the join table.
const mapKycServices = (kyc) => {
  if (!kyc) return kyc;
  const { serviceCategories, ...rest } = kyc;
  return { ...rest, services: (serviceCategories ?? []).map((row) => row.category) };
};
...
export const findVendorProfile = async (userId) => {
  ...
  return { ...profile, kyc: mapKycServices(profile.kyc) };
};
 ...
 */
export const upsertKycAndMarkSubmitted = async ({ vendorUserId, kyc, categoryIds }) => {
  return prisma.$transaction(async (tx) => {
    const created = await tx.vendorKyc.upsert({
      ...
      update: {
        ...
        // Replace the whole selection with whatever this submission sent.
        serviceCategories: {
          deleteMany: {},
    ...
    return { kyc: { ...mapKycServices(created), documents }, profile };
  });
};
...
export const approveKyc = async ({ vendorUserId, adminId }) => {
  return prisma.$transaction(async (tx) => {
    ...
    return { kyc: { ...mapKycServices(kyc), documents }, profile, user };
  });
};
...
export const listKycSubmissions = async ({ status, take, skip }) => {
  ...
  return { items: items.map((item) => ({ ...item, kyc: mapKycServices(item.kyc) })), total };
};
