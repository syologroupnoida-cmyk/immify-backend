...
export const submitMyKyc = async ({ vendorUserId, payload }) => {
  ...
  // `services` in the wizard payload is a list of ServiceCategory ids, not
  // free text — validate they exist before linking.
  const categoryIds = Array.from(new Set(payload.services ?? []));
  if (categoryIds.length > 0) {
    const existingIds = await categoryRepo.findExistingCategoryIds(categoryIds);
    const missingIds = categoryIds.filter((id) => !existingIds.includes(id));
    if (missingIds.length > 0) {
      throw ApiError.badRequest(`Unknown service category id(s): ${missingIds.join(', ')}`, {
