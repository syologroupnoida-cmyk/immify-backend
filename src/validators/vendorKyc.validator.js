...
export const submitKycSchema = z
  .object({
    ...
    // Business operations
    // `services` = ServiceCategory ids the vendor offers (not free text).
    services: z
      .array(z.string().trim().uuid('Each service must be a valid service category id'))
      .optional()
      .default([]),
...
export const listKycQuerySchema = z
  .object({
    ...
    // Dual-style pagination — same pattern as vendorManagement.
